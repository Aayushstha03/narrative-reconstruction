...
def parse_nepali_date(date_str):
    ...
    """
    nepali_months = {
        'बैशाख': 1, 'जेठ': 2, 'असार': 3, 'साउन': 4, 'भदौ': 5, 'आश्विन': 6, 'कार्तिक': 7, 'मंसिर': 8, 'पुष': 9, 'माघ': 10, 'फागुन': 11, 'चैत्र': 12,
        'Baisakh': 1, 'Jestha': 2, 'Ashar': 3, 'Shrawan': 4, 'Bhadra': 5, 'Ashwin': 6, 'Kartik': 7, 'Mangsir': 8, 'Poush': 9, 'Magh': 10, 'Falgun': 11, 'Chaitra': 12
    }
    import re
    date_part = date_str.split('गते')[0].strip()
    date_part = re.sub(r'[\d१२३४५६७८९०]{1,2}:\d{2}', '', date_part).strip()
    nepali_date_pattern = re.compile(r'([१२३४५६७८९०\d]{4})\s*([\w\u0900-\u097F]+)\s*([१२३४५६७८९०\d]{1,2})')
    ...
    if m:
        ...
        if month:
            try:
                ...
                nepali_days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']
...
def replace_dates_and_days_in_text(text):
    ...
    nepali_days = ['आइतबार', 'सोमबार', 'मंगलबार', 'बुधबार', 'बिहीबार', 'शुक्रबार', 'शनिबार']
    ...
    date_patterns = [
        re.compile(r'([१२३४५६७८९०\d]{4})\s*([\w\u0900-\u097F]+)\s*([१२३४५६७८९०\d]{1,2})\s*गते?'), # YYYY month DD
        re.compile(r'([\w\u0900-\u097F]+)\s*([१२३४५६७८९०\d]{1,2})'), # month DD
        re.compile(r'([१२३४५६७८९०\d]{1,2})\s*([\w\u0900-\u097F]+)')  # DD month
    ]
...
def main():
    ...
    for url in df['url']:
        ...
        if html:
            title, published_date_raw, content = extract_title_and_content(html)
        else:
